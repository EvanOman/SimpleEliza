...
while ($cond)
{
	...
	{
		my $goodbye = ">Good bye";
		$goodbye = $goodbye.", ".$info {"name"}.", I hope you do well in ".$info {"major"};
...
sub printMessage
{
	...
	#Don't want a loop, restart the conversation if I have said the same thing more than twice in a row
	if ($arrLength >= 3 && $outputArray[-1] eq $outputArray[-2] && $outputArray[-1] eq $outputArray[-3])
	{
