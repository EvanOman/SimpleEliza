...
while ($cond)
{
...

	{
...

		{
			...
			printMessage(">You sound discouraged. ".$info {
					"major"
				}.
				" is hard work but I think you can
...

			{
				...
				printMessage(">".$info {
					...
					", I am concerned that you do not know much about your

					graduation plan.If you want to succeed at ".$info{"
					major "}."
				...
				#
				Look
				for 'ask you a question'
				response

				elsif($input = ~/((ask you a| I have a) (\b(new|different|unrelated)\b\s)?question|ask you

					something(new | different | unrelated) ? ) / )
			...
			#
			The I(blank)
			case with the question response
...

	{
		...
		file: ///C:/users/evan~1.oma/appdata/local/temp/tmpye4u0r.html 2/4
	...
	#
	Gets and stores the name of the user and calls the getMajor
	function
...

	{
...

		{
...

			{
				...
				if ($input = ~s / ^ ((Hello, ? | Hi, ? ) ? ? ([Mm] y name is | I am | [Cc] all me | I 'm ))?
...

						{
							...
							printMessage(">Nice to meet you ".$input.
								", what is your major?\n");
...

						{
							...
							#
							If this is true then we looped
							for too long
...

				{
...

					{
...

						{
							...
							printMessage(">I have always been interested in ".$input.
